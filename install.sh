...
echo "${NEW_LINE}${GREEN}Download docker-compose.yml file${NO_COLOR}${NEW_LINE}"

curl -fsSL "${BASE_URL}/docker-compose.yml" > docker-compose.yml

echo "${NEW_LINE}${GREEN}Create docker/nginx directory${NO_COLOR}${NEW_LINE}"

mkdir -p docker/nginx

echo "${NEW_LINE}${GREEN}Download default.conf file${NO_COLOR}${NEW_LINE}"

curl -fsSL "${BASE_URL}/docker/nginx/default.conf" > docker/nginx/default.conf

echo "${NEW_LINE}${GREEN}Download general.conf file${NO_COLOR}${NEW_LINE}"

curl -fsSL "${BASE_URL}/docker/nginx/general.conf" > docker/nginx/general.conf

echo "${NEW_LINE}${GREEN}Download nginx.conf file${NO_COLOR}${NEW_LINE}"

curl -fsSL "${BASE_URL}/docker/nginx/nginx.conf" > docker/nginx/nginx.conf

echo "${NEW_LINE}${GREEN}Download php_fastcgi.conf file${NO_COLOR}${NEW_LINE}"

curl -fsSL "${BASE_URL}/docker/nginx/php_fastcgi.conf" > docker/nginx/php_fastcgi.conf

echo "${NEW_LINE}${GREEN}Done${NO_COLOR}${NEW_LINE}"
